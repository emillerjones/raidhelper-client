...
function formatRelativeRaidTime(date) {
  ...
  if (date.getDate() === tomorrow.getDate() && diffHours < 48) {
...
export default function Guilds() {
      ...
      try {
        ...
        const mappedEvents = allRaids.map((raid) => {
          ...
          return {
            ...
            localDate:
              raw.localDate ||
              getDateKeyFromTimestamp(raw.startTime || raid.start_time),
  ...
  const totalRaidsThisWeek = events.filter(
    (e) => e.localDate && e.localDate >= weekStart && e.localDate <= weekEnd
  ).length;

  const totalRaidsToday = events.filter(
    (e) => e.localDate === getDateKeyFromDate(today)
  ).length;
    ...
    <div className="guilds-page">
      <section className="guilds-hero">
        ...
        <p className="guilds-subhead">
          {totalGuilds} guilds tracked across Discord. Updates whenever a scan
          runs.
        </p>
      </section>
      ...
      <section className="guilds-panel">
        ...
        <div className="guilds-list">
          {guilds.map((guild) => {
            ...
            const nextRaidType = guild.nextRaid
              ? matchRaidType(guild.nextRaid.raid_name || guild.nextRaid.title)
              : null;

            const nextRaidColor = nextRaidType
              ? RAID_COLORS[nextRaidType]
              : null;

            return (
              <div
                className={
                  isOpen ? "guilds-row guilds-row--open" : "guilds-row"
                }
                key={guild.guild_id}
              >
                <button
                ...
                >
                  <div className="guilds-row-identity">
                    <GuildIcon
                      ...
                      guildIconUrl={guild.guildIconUrl}
                    />
                    <span className="guilds-row-name">
                  ...
                  <div className="guilds-row-stats">
                    <span className="guilds-row-week-value">
                      {guild.raidsThisWeek}
                      <span className="guilds-stat-label"> this week </span>
                    </span>

                    <span className="guilds-row-total-value">
                      {guild.raids.length}
                      <span className="guilds-stat-label"> total </span>
                    </span>
                    ...
                    {guild.nextRaid ? (
                      ...
                      >
                        <span className="guilds-row-next-badge">
                          {nextRaidType}
                        </span>
                        <span className="guilds-row-next-time">
                          {formatRelativeRaidTime(
                            new Date(
                              guild.nextRaid.startTime ||
                                guild.nextRaid.start_time
                            )
                          )}
                        </span>
                      </a>
                    ) : (
                      <span className="guilds-row-next guilds-row-next--empty">
                        No upcoming raid
                      </span>
                    )}
                  </div>
                ...
                {isOpen && (
                  <div className="guilds-row-body">
                    ...
                    {guild.raids
                      ...
                      .map((raid) => {
                        const raidType = matchRaidType(
                          raid.raid_name || raid.title
                        );

                        const signupCount =
                          raid.signup_count ?? raid.raw_json?.signupCount ?? "?";

                        const signupMax =
                          raid.signup_max ?? raid.raw_json?.signupMax ?? "?";

                        const creator =
                          raid.raid_leader ??
                          raid.raw_json?.raidLeader ??
                          "Unknown";
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="guilds-raid-name-wrap">
                              <span className="guilds-raid-name">
                                {raid.raid_name || raid.title}
                              </span>
                              <span className="guilds-raid-creator">
                                by {creator}
                              </span>
                            </span>

                            <span className="guilds-raid-signups">
                              {signupCount}/{signupMax}
                            </span>

                            <span className="guilds-raid-date">
                              {new Date(
                                raid.startTime || raid.start_time
                              ).toLocaleDateString([], {
                                weekday: "short",
                                month: "short",
                                day: "numeric",
